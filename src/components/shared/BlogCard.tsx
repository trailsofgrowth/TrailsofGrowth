...
export default function BlogCard({ title, author, date, category, excerpt, slug, gradient, featured_image }: BlogCardProps) {
  return (
    <div className="bg-white rounded-xl shadow-sm hover:shadow-md hover:-translate-y-1 transition-all overflow-hidden">
      {/* Image */}
      {featured_image ? (
        <img src={featured_image} alt={title} className="w-full h-44 object-cover" />
      ) : (
